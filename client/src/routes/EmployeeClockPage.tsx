...
type GpsStatus = 'verified' | 'outside' | 'locating'

function GPSBadge({ status }: { status: GpsStatus }) {
  const config = {
    verified: { label: 'On-site verified', Icon: CheckCircle, className: 'text-emerald-600 dark:text-emerald-400 bg-emerald-500/10 dark:bg-emerald-500/15 border-emerald-500/30' },
    outside: { label: 'Outside geofence', Icon: AlertTriangle, className: 'text-amber-600 dark:text-amber-400 bg-amber-500/10 dark:bg-amber-500/15 border-amber-500/30' },
    locating: { label: 'Locating…', Icon: MapPin, className: 'text-primary bg-primary/10 dark:bg-primary/20 border-primary/30' },
  }[status]
  const { label, Icon, className } = config
  return (
    <div className={`inline-flex items-center gap-1.5 rounded-full border px-3.5 py-1.5 text-xs font-semibold tracking-wide ${className}`}>
...
export function EmployeeClockPage() {
  ...
  useEffect(() => {
    if (!location || !geofence) {
      if (location && selectedJobId && !geofence) setGpsStatus('verified')
      return
    }
    const inside =
      distanceMeters(geofence.center_lat, geofence.center_lng, location.lat, location.lng) <=
      (geofence.radius_unit === 'meters' ? geofence.radius_value : feetToMeters(geofence.radius_value))
    setGpsStatus(inside ? 'verified' : 'outside')
  }, [location, geofence, selectedJobId])

  useEffect(() => {
    if (!location && !locationError) return
    if (locationError) setGpsStatus('locating')
    else if (!geofence && selectedJobId) setGpsStatus('verified')
  }, [location, locationError, geofence, selectedJobId])
      ...
      <div className="relative z-10">
          ...
          <>
            ...
            {geofence && isInsideGeofence === false && (
              <div className="flex items-center gap-2 rounded-xl border border-amber-500/20 bg-amber-500/10 dark:bg-amber-500/15 p-3 mt-4">
                ...
                <span className="text-xs text-amber-600 dark:text-amber-400">You must be inside the job site to clock in.</span>
              </div>
            )}
          </>
        )}
        ...
        {!clockedIn && step === 'jobs' && (
          <>
            ...
            <div className="clock-fade-up flex justify-center mb-2" style={{ animationDelay: '0.12s' }}>
              <GPSBadge status={locationError ? 'locating' : gpsStatus} />
            </div>

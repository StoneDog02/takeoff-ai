...
/**
 * @param {{ clientName: string, gcName: string, projectName: string, portalUrl: string }} opts
 * @returns {{ subject: string, html: string, text: string }}
 */
function renderEstimatePortalEmail({ clientName, gcName, projectName, portalUrl }) {
  const name = clientName || 'there'
  const gc = gcName || 'Your contractor'
  const project = projectName || 'your project'
  const subject = `Estimate ready for ${project}`
...
<body style="margin:0;padding:0;background-color:#f7f6f3;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,'Helvetica Neue',Arial,sans-serif;font-size:16px;line-height:1.5;color:#1c1a18;">
  <div class="wrapper" style="max-width:600px;margin:0 auto;padding:40px 24px;">
    <div class="card" style="background:#ffffff;border-radius:14px;border:1px solid rgba(0,0,0,0.07);padding:32px 28px;">
      ...
      <p class="body-text" style="font-size:15px;color:#6b6764;margin:0 0 24px;line-height:1.55;">${escapeHtml(gc)} has sent you an estimate for <strong>${escapeHtml(project)}</strong>. Review the details and approve, request changes, or decline from the link below.</p>
      <div class="cta-wrap" style="margin:28px 0 0;">
        <a href="${escapeHtml(portalUrl)}" class="cta" style="display:inline-block;padding:14px 28px;background:#c0392b;color:#ffffff;font-size:14px;font-weight:700;text-decoration:none;border-radius:10px;">Review estimate</a>
      </div>
...
${gc} has sent you an estimate for ${project}. Review the details and approve, request changes, or decline.

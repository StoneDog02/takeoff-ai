/**
 * Email template: client invoice portal link (send / resend).
 */
...
<body style="margin:0;padding:0;background-color:#f7f6f3;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,'Helvetica Neue',Arial,sans-serif;font-size:16px;line-height:1.5;color:#1c1a18;">
  <div class="wrapper" style="max-width:600px;margin:0 auto;padding:40px 24px;">
    <div class="card" style="background:#ffffff;border-radius:14px;border:1px solid rgba(0,0,0,0.07);padding:32px 28px;">
      <h1 class="heading" style="font-size:22px;font-weight:600;color:#1c1a18;margin:0 0 8px;">Hi ${escapeHtml(who)},</h1>
      ...
      <div class="cta-wrap" style="margin:28px 0 0;">
        <a href="${escapeHtml(portalUrl)}" class="cta" style="display:inline-block;padding:14px 28px;background:#1f4a8a;color:#ffffff;font-size:14px;font-weight:700;text-decoration:none;border-radius:10px;">View invoice</a>
      </div>
      <p class="muted" style="font-size:13px;color:#b0ada9;margin:24px 0 0;">If the button doesn’t work, copy and paste this link:<br/><a href="${escapeHtml(portalUrl)}" style="color:#2d6fa8;word-break:break-all;">${escapeHtml(portalUrl)}</a></p>
    </div>
...
module.exports = { renderInvoicePortalEmail }

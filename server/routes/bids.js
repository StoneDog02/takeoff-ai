...
router.get('/portal/:token', async (req, res, next) => {
  try {
    ...
    const { data: bid, error: bidErr } = await supabase
      .from('sub_bids')
      .select('id, trade_package_id, subcontractor_id, amount, notes, availability, quote_url, awarded, response_status, responded_at')
    ...
    const { data: proj } = projectId
      ? await supabase.from('projects').select('id, name, status, address_line_1, address_line_2, city, state, postal_code').eq('id', projectId).maybeSingle()
      : { data: null }
    ...
    const address = proj ? [proj.address_line_1, proj.address_line_2, proj.city, proj.state, proj.postal_code].filter(Boolean).join(', ') : ''
